...
/** Simulates a basket of stocks locally for demo mode */
function useStocksSimMulti(initialPrice = 100, stepMs = 60 * 1000, symbols = ["AAPL", "MSFT", "NVDA", "AMZN", "TSLA"]) {
  ...
  // One shared portfolio across symbols (simple, gamey)
  ...
  function tickOne({ driftBps = 1, shockProb = 0.01, trendProb = 0.6 } = {}) {
    // Evolve each symbol once per tick
    setOhlcMap((prev) => {
      const nextMap = { ...prev };
    ...
    // Recalculate equity with latest prices
...
export default function TradeDemo({
  ...
  useRemoteStocks = false, // set true when you have a remote stocks API
  ...
  /* ----------------------------- State ---------------------------- */
  const [venue, setVenue] = useState(defaultVenue); // "dex" | "cex" | "both" | "stocks" | "bundle"
  const [chain, setChain] = useState("ethereum");
  const [symbols, setSymbols] = useState(defaultSymbols || "BTC,ETH");
  const [stockSymbols, setStockSymbols] = useState("AAPL,MSFT,NVDA,AMZN,TSLA"); // more stocks by default

  // Crypto strategy catalog (renamed & simplified labels)
  ...
  // Stocks strategy controls (renamed & simplified)
  const [smaFast, setSmaFast] = useState(10);     // Fast Average
  const [smaSlow, setSmaSlow] = useState(30);     // Slow Average
  const [rsiWindow, setRsiWindow] = useState(14); // RSI Lookback
  const [rsiTop, setRsiTop] = useState(70);       // RSI Overbought
  const [rsiBottom, setRsiBottom] = useState(30); // RSI Oversold
  const [stockTradeUnits, setStockTradeUnits] = useState(10);

  // Stocks realism knobs (renamed)
  const [feeBps, setFeeBps] = useState(5);          // Fees (bps)
  const [spreadBps, setSpreadBps] = useState(8);    // Bid-Ask Spread (bps)
  const [slipBps, setSlipBps] = useState(10);       // Slippage (bps)
  const [latencyMs, setLatencyMs] = useState(80);   // Fill Delay (ms)
  const [driftBps, setDriftBps] = useState(1);      // Gentle Drift
  const [shockProb, setShockProb] = useState(0.01); // Surprise Jumps
  const [trendProb, setTrendProb] = useState(0.6);  // Trending Regime

  // Sessions returned by backend / or local sim
  ...
  const sim = useStocksSimMulti(100, 60 * 1000, stockList);
  ...
  const [showAutoHint, setShowAutoHint] = useState(false); // << nudge to click Auto run
  ...
  const BOTH = venue === "both";
  const BUNDLE = venue === "bundle";
  ...
  function localStocksTick() {
    ...
    // trade a few names per bar for motion
  ...
  async function startOne(kind) {
    // STOCKS: use remote if enabled & reachable; else local basket sim
    if (kind === "stocks") {
  ...
  const handleStart = async () => {
    ...
    if (!usingDemo && !isLiveEligible && venue !== "stocks" && venue !== "bundle") {
      setBusy(false);
      setShowUpgrade(true);
      return;
    }
    ...
    try {
      ...
      // Show helpful hint to start Auto run
  ...
  return (
    <div className="w-full bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
      {/* Sticky Progress Bar (visible when running) */}
      {isRunning && (
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 pt-3 pb-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            <ModeToggle runMode={runMode} setRunMode={setRunMode} isLiveEligible={isLiveEligible || venue === "stocks" || venue === "bundle"} onUpgrade={() => setShowUpgrade(true)} />
          </div>
        </div>
          ...
          <div className="space-y-4 rounded-2xl border border-slate-600/60 bg-slate-900/90 p-3 sm:p-4">
              ...
              {(venue === "stocks" || venue === "bundle") && (
                <FieldCard title="Execution & Market Feel" help="Make fills more/less realistic.">
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <label title="Exchange fees in basis points (100 bps = 1%)">
                      Fees (bps)
                      <input
                        type="number"
                        min="0"
                    ...
                    <label title="Distance between best bid/ask">
                      Spread (bps)
                      <input
                        type="number"
                        min="0"
                    ...
                    <label title="Extra price movement during fills">
                      Slippage (bps)
                      <input
                        type="number"
                        min="0"
                    ...
                    <label title="Delay to fill orders">
                      Fill Delay (ms)
                      <input
                        type="number"
                        min="0"
                    ...
                    <label title="Gentle drift per bar">
                      Drift (bps/bar)
                      <input
                        type="number"
                        value={driftBps}
                    ...
                    <label title="Chance of a surprise jump">
                      Surprise Jumps (0–1)
                      <input
                        type="number"
                        step="0.001"
                        ...
                        max="1"
                        value={shockProb}
                        onChange={(e) => setShockProb(Number(e.target.value))}
                        className="w-full mt-1 border border-slate-600/60 rounded bg-slate-950 px-2 py-1"
                      />
                    </label>
                    <label title="How often market trends vs. mean-reverts">
                      Trend Regime (0–1)
                      <input
                        type="number"
                        step="0.01"
                        ...
                        max="1"
                        value={trendProb}
                        onChange={(e) => setTrendProb(Number(e.target.value))}
                        className="w-full mt-1 border border-slate-600/60 rounded bg-slate-950 px-2 py-1"
                      />
                    </label>
                  </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <button onClick={handleStart} disabled={busy} className="w-full sm:flex-1 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 border border-emerald-400 font-semibold">
                {busy ? "Starting…" : `Start ${usingDemo ? "Demo" : "Live"} (Step 3)`}
              </button>
...
function ModeToggle({ runMode, setRunMode, isLiveEligible, onUpgrade }) {
  const isLive = runMode === "live";
  return (
