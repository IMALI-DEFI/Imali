...
import { 
  ...
  FiChevronUp, FiChevronDown, FiEye, FiShieldOff
} from 'react-icons/fi';

const API_BASE = process.env.REACT_APP_API_BASE_URL || 'https://api.imali-defi.com';

// Helper to get auth token - FIXED: Use correct token key
const getAuthToken = () => {
...
const checkAdminStatus = async () => {
  try {
    ...
    return response.data?.data?.is_admin === true;
  } catch (error) {
...
const ReferralAnalytics = ({ 
  ...
  // Fetch referral analytics (only if admin)
  const fetchReferralAnalytics = useCallback(async () => {
    ...
    try {
      ...
      if (earningsRes.data.success) {
        setReferralStats(earningsRes.data.data?.summary || earningsRes.data.data);
      ...
      if (error.response?.status === 401) {
        ...
        showToast('Admin access required to view referral analytics', 'error');
      } else {
  ...
  }, [isAdmin, adminCheckDone, fetchReferralAnalytics]);
  ...
  if (adminCheckDone && !isAdmin) {
    return (
      <div className="flex h-96 flex-col items-center justify-center">
        <FiShieldOff className="mx-auto mb-4 h-16 w-16 text-red-400" />
        <h3 className="mb-2 text-xl font-semibold text-white">Admin Access Required</h3>
        <p className="text-center text-white/60">
          You don't have permission to view referral analytics.
        </p>
      ...
      {referralStats && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            <div className="mt-2 text-xs text-white/40">
              ${(referralStats.paid_out || 0).toFixed(2)} paid
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-2xl font-bold">${(referralStats.pending_rewards || 0).toFixed(2)}</span>
            </div>
            <div className="mt-2 text-xs text-white/40">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <div className="flex flex-wrap gap-3">
                  <button
                    ...
                    disabled={processing || (referralStats.pending_rewards || 0) < 50}
                ...
                <div className="space-y-3">
                  <div>
                    <div className="mb-1 flex justify-between text-sm">
                      ...
                      <span>${(referralStats.paid_out || 0).toFixed(2)}</span>
                    </div>
                    <div className="h-2 overflow-hidden rounded-full bg-white/10">
                      <div 
                        className="h-full rounded-full bg-green-500 transition-all"
                        style={{ 
                          width: `${Math.min(100, ((referralStats.paid_out || 0) / (referralStats.total_rewards_earned || 1)) * 100)}%` 
                        }}
                      />
                    </div>
                  </div>
                  <div>
                    <div className="mb-1 flex justify-between text-sm">
                      ...
                      <span>${(referralStats.pending_rewards || 0).toFixed(2)}</span>
                    </div>
                    <div className="h-2 overflow-hidden rounded-full bg-white/10">
                      <div 
                        className="h-full rounded-full bg-yellow-500 transition-all"
                        style={{ 
                          width: `${Math.min(100, ((referralStats.pending_rewards || 0) / (referralStats.total_rewards_earned || 1)) * 100)}%` 
                        }}
                      />
                    </div>
                  </div>
                        ...
                        <div className="mt-3 pt-3 border-t border-white/10">
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            ...
                            <div>
                              ...
                              <p>{referrer.referrals > 0 ? Math.round((referrer.rewards_earned / (referrer.fees_generated * 0.2)) * 100) : 0}%</p>
                            </div>
                          </div>
                        </div>
                      )}
