    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="max-h-[92vh] w-full max-w-6xl overflow-auto rounded-3xl bg-white p-6 shadow-2xl">
        <div className="mb-5 flex items-start justify-between gap-4 border-b border-slate-200 pb-4">
          ...
          <button onClick={onClose} className="rounded-xl px-3 py-1 text-3xl font-extrabold text-slate-500 hover:bg-slate-100 hover:text-slate-900">
            ×
          </button>
        </div>
          ...
          <Card className="bg-slate-50">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <KeyBox
                title="Paper Keys"
                fields={[
                  { placeholder: "API Key", value: alpacaPaper.apiKey, onChange: (v) => setAlpacaPaper({ ...alpacaPaper, apiKey: v }) },
                  { placeholder: "Secret Key", value: alpacaPaper.secret, type: "password", onChange: (v) => setAlpacaPaper({ ...alpacaPaper, secret: v }) },
                ]}
              ...
              <KeyBox
                title="Live Keys"
                fields={[
                  { placeholder: "API Key", value: alpacaLive.apiKey, onChange: (v) => setAlpacaLive({ ...alpacaLive, apiKey: v }) },
                  { placeholder: "Secret Key", value: alpacaLive.secret, type: "password", onChange: (v) => setAlpacaLive({ ...alpacaLive, secret: v }) },
                ]}
          ...
          <Card className="bg-slate-50">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <KeyBox
                title="Paper Keys"
                fields={[
                  { placeholder: "API Key", value: okxPaper.apiKey, onChange: (v) => setOkxPaper({ ...okxPaper, apiKey: v }) },
                  { placeholder: "Secret Key", value: okxPaper.secret, type: "password", onChange: (v) => setOkxPaper({ ...okxPaper, secret: v }) },
                  { placeholder: "Passphrase", value: okxPaper.passphrase, type: "password", onChange: (v) => setOkxPaper({ ...okxPaper, passphrase: v }) },
                ]}
                button="Save Paper Keys"
              ...
              <KeyBox
                title="Live Keys"
                fields={[
                  { placeholder: "API Key", value: okxLive.apiKey, onChange: (v) => setOkxLive({ ...okxLive, apiKey: v }) },
                  { placeholder: "Secret Key", value: okxLive.secret, type: "password", onChange: (v) => setOkxLive({ ...okxLive, secret: v }) },
                  { placeholder: "Passphrase", value: okxLive.passphrase, type: "password", onChange: (v) => setOkxLive({ ...okxLive, passphrase: v }) },
                ]}
                button="Save Live Keys"
...
export default function MemberDashboard() {
  ...
  const loadDashboard = async (silent = false) => {
    ...
    try {
      ...
      setTrial(trialPayload || { trial_status: "trial", paper_trading_enabled: me?.paper_trading_enabled === true, seconds_remaining: 0 });
    } catch (err) {
  ...
  const lineData = useMemo(() => ({
    ...
    datasets: [{ label: "PnL", data: series.map((p) => Number(p.pnl || 0)), borderColor: "#4f46e5", backgroundColor: "rgba(79, 70, 229, 0.12)", fill: true, tension: 0.35 }],
  }), [series]);
  ...
  const chartOptions = {
    ...
    scales: { x: { ticks: { color: "#334155" }, grid: { color: "rgba(148, 163, 184, 0.25)" } }, y: { ticks: { color: "#334155" }, grid: { color: "rgba(148, 163, 184, 0.25)" } } },
  };

  const handleTogglePaperTrading = async (enabled) => {
    if (enabled && !bothConnected) { setShowApiModal(true); return; }
    setTogglingPaper(true);
    try {
      setPaperTradingEnabled(enabled);
      setUser((prev) => prev ? { ...prev, paper_trading_enabled: enabled } : prev);
      setStats((prev) => ({ ...prev, total_trades: Math.max(Number(prev.total_trades || 0), enabled ? 1 : 0) }));
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex flex-col gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            <SecondaryButton onClick={() => loadDashboard(true)} disabled={refreshing}>{refreshing ? "Refreshing..." : "Refresh"}</SecondaryButton>
            <SecondaryButton onClick={() => setShowApiModal(true)}>Connect API Keys</SecondaryButton>
            <SecondaryButton onClick={() => nav("/billing-dashboard")}>Billing</SecondaryButton>
          </div>
        </div>

        {/* Next Step Card */}
        <div className={`rounded-3xl border p-6 shadow-sm ${bothConnected && !paperTradingEnabled && !tradingEnabled ? "border-blue-300 bg-blue-50" : paperTradingEnabled && !tradingEnabled ? "border-green-300 bg-green-50" : tradingEnabled ? "border-purple-300 bg-purple-50" : "border-amber-300 bg-amber-50"}`}>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 className={`text-2xl font-extrabold ${bothConnected && !paperTradingEnabled && !tradingEnabled ? "text-blue-950" : paperTradingEnabled && !tradingEnabled ? "text-green-950" : tradingEnabled ? "text-purple-950" : "text-amber-950"}`}>
              ...
              <p className={`mt-2 max-w-3xl text-base font-semibold ${bothConnected && !paperTradingEnabled && !tradingEnabled ? "text-blue-800" : paperTradingEnabled && !tradingEnabled ? "text-green-800" : tradingEnabled ? "text-purple-800" : "text-amber-800"}`}>
                {!bothConnected ? "Connect both Alpaca and OKX so Imali can run paper trading and live trading." : !paperTradingEnabled ? "Your accounts are connected. Start with virtual money first." : !tradingEnabled ? "Imali is using virtual funds. Watch your charts, trades, and strategy before using real money." : "Real money trading is turned on. Monitor performance and stop live trading anytime."}
              </p>
              <div className="mt-4 flex flex-wrap gap-2">
            ...
            <div className="shrink-0">
              {!bothConnected ? (
                <WarningButton onClick={() => setShowApiModal(true)}>Connect API Keys</WarningButton>
              ) : !paperTradingEnabled ? (
                <PrimaryButton onClick={() => handleTogglePaperTrading(true)} disabled={togglingPaper}>Start Paper Trading</PrimaryButton>
              ) : !tradingEnabled ? (
                <div className="flex flex-wrap gap-3">
                  <DangerButton onClick={() => handleTogglePaperTrading(false)} disabled={togglingPaper}>Stop Paper</DangerButton>
                  <WarningButton onClick={() => setShowLiveConfirm(true)} disabled={togglingTrading}>Start Live Trading</WarningButton>
                </div>
              ) : (
                <DangerButton onClick={() => handleToggleTrading(false)} disabled={togglingTrading}>Stop Live Trading</DangerButton>
              )}
            </div>
          </div>
        </div>
        ...
        <Card className="border-indigo-200 bg-indigo-50">
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <div className="rounded-2xl border border-indigo-100 bg-white p-4">
              <div className="mb-3 flex h-8 w-8 items-center justify-center rounded-full bg-indigo-600 text-sm font-extrabold text-white">1</div>
              ...
              <button onClick={() => setShowApiModal(true)} className="mt-3 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs font-extrabold text-indigo-800 hover:bg-indigo-100">Connect Keys</button>
            </div>
            <div className="rounded-2xl border border-indigo-100 bg-white p-4">
              <div className="mb-3 flex h-8 w-8 items-center justify-center rounded-full bg-indigo-600 text-sm font-extrabold text-white">2</div>
              <div className="text-base font-extrabold text-slate-950">Start paper trading</div>
              <p className="mt-1 text-sm font-medium text-slate-600">Practice with ${PAPER_TRADING_BALANCE.toLocaleString()} virtual funds.</p>
              <button onClick={() => handleTogglePaperTrading(true)} disabled={!bothConnected} className="mt-3 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs font-extrabold text-indigo-800 hover:bg-indigo-100">Paper Trade</button>
            </div>
            <div className="rounded-2xl border border-indigo-100 bg-white p-4">
              <div className="mb-3 flex h-8 w-8 items-center justify-center rounded-full bg-indigo-600 text-sm font-extrabold text-white">3</div>
              <div className="text-base font-extrabold text-slate-950">Choose a strategy</div>
              <p className="mt-1 text-sm font-medium text-slate-600">Pick Conservative, Balanced, Momentum, or Arbitrage.</p>
            </div>
            <div className="rounded-2xl border border-indigo-100 bg-white p-4">
              <div className="mb-3 flex h-8 w-8 items-center justify-center rounded-full bg-indigo-600 text-sm font-extrabold text-white">4</div>
              <div className="text-base font-extrabold text-slate-950">Go live when ready</div>
              <p className="mt-1 text-sm font-medium text-slate-600">Turn on live trading after testing with paper trading.</p>
              <button onClick={() => setShowLiveConfirm(true)} disabled={!paperTradingEnabled} className="mt-3 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs font-extrabold text-indigo-800 hover:bg-indigo-100">Go Live</button>
            </div>
          </div>
        </Card>
        ...
        <div className="grid gap-5 xl:grid-cols-2">
          <Card className={paperTradingEnabled ? "border-green-300 bg-green-50" : !bothConnected ? "bg-slate-50 opacity-90" : "bg-white"}>
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <div className="flex items-center gap-3"><span className="text-3xl">📝</span><h3 className="text-xl font-extrabold text-slate-950">Paper Trading</h3></div>
                <p className="mt-2 text-sm font-semibold text-slate-600">Practice with ${PAPER_TRADING_BALANCE.toLocaleString()} virtual money. No real money is used.</p>
                ...
                <div className="mt-4"><StatusPill tone={paperTradingEnabled ? "green" : "slate"}>{paperTradingEnabled ? "Paper Trading Active" : bothConnected ? "Ready to Start" : "Connect Keys First"}</StatusPill></div>
              </div>
              <div className="shrink-0">
                {paperTradingEnabled ? (
                  <DangerButton onClick={() => handleTogglePaperTrading(false)}>Stop</DangerButton>
                ) : (
                  <PrimaryButton onClick={() => handleTogglePaperTrading(true)} disabled={!bothConnected}>Start</PrimaryButton>
                )}
              </div>
            </div>
          </Card>
          ...
          <Card className={tradingEnabled ? "border-green-300 bg-green-50" : "bg-white"}>
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <div className="flex items-center gap-3"><span className="text-3xl">💰</span><h3 className="text-xl font-extrabold text-slate-950">Live Trading</h3></div>
                <p className="mt-2 text-sm font-semibold text-slate-600">Trade with real funds through your connected exchange accounts.</p>
                {!bothConnected && <p className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm font-bold text-amber-900">Connect both OKX and Alpaca first.</p>}
                <div className="mt-4"><StatusPill tone={tradingEnabled ? "green" : "slate"}>{tradingEnabled ? "Live Trading Active" : bothConnected ? "Ready When You Are" : "Connect Keys First"}</StatusPill></div>
              </div>
              <div className="shrink-0">
                {tradingEnabled ? (
                  <DangerButton onClick={() => handleToggleTrading(false)}>Stop</DangerButton>
                ) : (
                  <WarningButton onClick={() => setShowLiveConfirm(true)} disabled={!bothConnected}>Start</WarningButton>
                )}
              </div>
            </div>
          </Card>
        ...
        <Card>
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div><SectionTitle>📊 Trading Readiness</SectionTitle><p className="-mt-2 text-sm font-semibold text-slate-600">This score helps beginners know how complete their setup is.</p></div>
        ...
        <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">
          <Stat label="Total Profit" value={usd(displayStats.total_pnl)} helper={tradingEnabled ? "Live + closed trades" : "Paper/live closed trades"} />
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {STRATEGIES.map((strategy) => (
              <button
                ...
                className={`w-full rounded-2xl border p-5 text-left transition hover:-translate-y-0.5 hover:shadow-md disabled:opacity-70 ${currentStrategy === strategy.id ? "border-indigo-500 bg-indigo-50 ring-2 ring-indigo-200" : "border-slate-200 bg-white"}`}
              >
                <div className="mb-3 flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3"><span className="text-3xl">{strategy.emoji}</span><div><div className="text-lg font-extrabold text-slate-950">{strategy.name}</div><div className="text-xs font-bold text-slate-500">Best for: {strategy.bestFor}</div></div></div>
                  <span className={`rounded-full border px-2 py-1 text-xs font-extrabold ${riskClass(strategy.risk)}`}>{strategy.risk}</span>
                </div>
                ...
                <div className="mt-4 border-t border-slate-100 pt-3">
                  <StatusPill tone={currentStrategy === strategy.id ? "purple" : "slate"}>{currentStrategy === strategy.id ? (savingStrategy === strategy.id ? "Updating..." : "Active Strategy") : "Tap to use"}</StatusPill>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="grid gap-5 xl:grid-cols-3">
          <Card className="xl:col-span-2"><SectionTitle>📈 PnL Performance</SectionTitle><div className="h-72"><Line data={lineData} options={chartOptions} /></div></Card>
          <Card><SectionTitle>🥇 Win / Loss</SectionTitle><div className="h-72"><Doughnut data={doughnutData} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { color: "#0f172a", font: { weight: "bold" } } } } }} /></div></Card>
        </div>

        <Card><SectionTitle>📊 Trade Count — Last 7 Days</SectionTitle><div className="h-72"><Bar data={barData} options={chartOptions} /></div></Card>
          ...
          <Card>
            ...
            <div className="space-y-3">
              <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
                <div><div className="text-base font-extrabold text-slate-950">Alpaca</div><div className="text-sm font-medium text-slate-500">Needed for stock trading.</div></div>
                <div className="flex items-center gap-3"><StatusPill tone={alpacaConnected ? "green" : "amber"}>{alpacaConnected ? "Connected" : "Needed"}</StatusPill>{!alpacaConnected && <SecondaryButton onClick={() => setShowApiModal(true)} className="px-3 py-2">Connect</SecondaryButton>}</div>
              </div>
              <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
                <div><div className="text-base font-extrabold text-slate-950">OKX</div><div className="text-sm font-medium text-slate-500">Needed for crypto trading.</div></div>
                <div className="flex items-center gap-3"><StatusPill tone={okxConnected ? "green" : "amber"}>{okxConnected ? "Connected" : "Needed"}</StatusPill>{!okxConnected && <SecondaryButton onClick={() => setShowApiModal(true)} className="px-3 py-2">Connect</SecondaryButton>}</div>
              </div>
              <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
                <div><div className="text-base font-extrabold text-slate-950">Wallet</div><div className="text-sm font-medium text-slate-500">Optional for DeFi features.</div></div>
                <div className="flex items-center gap-3"><StatusPill tone={integrations.wallet_connected ? "green" : "slate"}>{integrations.wallet_connected ? "Connected" : "Optional"}</StatusPill></div>
              </div>
            </div>
          </Card>
                ...
                {communityTrades.map((trade, index) => {
                  ...
                  return (
                    <div key={trade.id || index} className="flex items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-4">
                      ...
                      <div className={`text-sm font-extrabold ${positive ? "text-green-700" : "text-red-700"}`}>{usd(pnl)}</div>
                    </div>
                  );
                })}
              </div>
            ...
            {[
              ...
              <div key={feature.title} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div><div className="text-base font-extrabold text-slate-950">{feature.title}</div><div className="mt-1 text-sm font-semibold text-slate-600">{feature.desc}</div></div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <PrimaryButton onClick={() => nav("/trade-demo")}>Paper Trade Demo</PrimaryButton>
          <WarningButton onClick={() => bothConnected ? setShowLiveConfirm(true) : setShowApiModal(true)}>Start Live Trading</WarningButton>
          <SecondaryButton onClick={() => nav("/activation")}>Complete Activation</SecondaryButton>
        </div>
      </div>

      <ApiKeysModal open={showApiModal} onClose={() => setShowApiModal(false)} onSaved={() => loadDashboard(true)} />
      <LiveConfirmModal open={showLiveConfirm} onCancel={() => setShowLiveConfirm(false)} onConfirm={() => handleToggleTrading(true)} />
    </div>
  );
}
