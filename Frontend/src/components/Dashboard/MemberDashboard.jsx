...
// Register ChartJS components (no external plugins needed)
ChartJS.register(
  CategoryScale,
  LinearScale,
  PointElement,
...
export default function MemberDashboard() {
  ...
  const loadDashboard = useCallback(async ({ silent = false, force = false } = {}) => {
    ...
    try {
      ...
      if (Array.isArray(tradesPayload?.trades) && tradesPayload.trades.length > 0) setCommunityTrades(tradesPayload.trades);
  ...
  // ========== CHART CONFIGURATIONS (Compatible with chart.js v4.5.1) ==========
  ...
  const lineChartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        titleColor: "#ffffff",
        bodyColor: "#e2e8f0",
        callbacks: { label: (context) => `P&L: ${context.raw > 0 ? "+" : ""}$${context.raw.toFixed(2)}` },
      },
    },
    scales: {
      x: { ticks: { color: "#475569", font: { size: 10 } }, grid: { color: "rgba(148, 163, 184, 0.15)" } },
      y: { ticks: { color: "#475569", callback: (value) => `$${value}` }, grid: { color: "rgba(148, 163, 184, 0.15)" }, title: { display: true, text: "Profit & Loss ($)", color: "#64748b", font: { size: 10 } } },
    },
  };
  ...
  const doughnutChartOptions = {
    ...
    cutout: "60%",
    plugins: {
      legend: { position: "bottom", labels: { color: "#1e293b", font: { weight: "bold", size: 11 } } },
      tooltip: {
        backgroundColor: "#1e293b",
        titleColor: "#ffffff",
        bodyColor: "#e2e8f0",
        callbacks: { label: (context) => `${context.label}: ${context.raw} trades` },
      },
    },
  };
  ...
  const barChartOptions = {
    ...
    plugins: {
      legend: { position: "top", labels: { color: "#1e293b", font: { weight: "bold", size: 11 } } },
      tooltip: { backgroundColor: "#1e293b", titleColor: "#ffffff", bodyColor: "#e2e8f0", callbacks: { label: (context) => `${context.raw} trades` } },
    },
    scales: {
      x: { ticks: { color: "#475569", font: { size: 10, maxRotation: 45, minRotation: 45 } }, grid: { display: false } },
      y: { ticks: { color: "#475569", stepSize: 5 }, grid: { color: "rgba(148, 163, 184, 0.15)" }, title: { display: true, text: "Number of Trades", color: "#64748b", font: { size: 10 } } },
    },
  };
  ...
  // The rest of the JSX remains the same as your original working version
  // (keeping the same layout structure for brevity - the charts sections are updated above)
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-3 py-4 text-slate-900 sm:p-6">
      ...
      <div className="mx-auto max-w-7xl space-y-5 sm:space-y-6">
            ...
            <div className="w-full shrink-0 lg:w-auto">
              {!bothConnected ? (
                ...
                <div className="grid gap-3 sm:grid-cols-3 lg:flex">
                  ...
                  <Button variant="primary" onClick={async () => { await executePaperTrade(); notify("Manual trade executed!", "success"); }} disabled={executingTrade || !paperTradingEnabled} className="w-full lg:w-auto">
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {[
              ...
              <div key={item.step} className="rounded-2xl border border-indigo-100 bg-white p-4">
                ...
                {item.action && (
                  <button onClick={item.onClick} disabled={item.disabled} className="mt-3 min-h-[40px] w-full rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs font-extrabold text-indigo-800 hover:bg-indigo-100 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto">
                    {item.action}
                  </button>
                )}
              </div>
        ...
        <div className="grid gap-5 xl:grid-cols-2">
          <Card className={paperTradingEnabled ? "border-green-300 bg-green-50" : "bg-white"}>
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <div className="flex items-center gap-3"><span className="text-3xl">📝</span><h3 className="text-lg font-extrabold text-slate-900 sm:text-xl">Paper Trading</h3></div>
              ...
              <div className="w-full shrink-0 sm:w-auto">
                {paperTradingEnabled ? (
                  <div className="flex flex-col gap-2">
                    ...
                    <Button variant="primary" onClick={async () => { await executePaperTrade(); notify("Manual trade executed!", "success"); }} disabled={executingTrade} className="w-full">{executingTrade ? "Trading..." : "Manual Trade"}</Button>
                  </div>
                ) : (
          ...
          <Card className={tradingEnabled ? "border-green-300 bg-green-50" : "bg-white"}>
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <div className="flex items-center gap-3"><span className="text-3xl">💰</span><h3 className="text-lg font-extrabold text-slate-900 sm:text-xl">Live Trading</h3></div>
        ...
        {/* Charts Grid - Compatible with chart.js v4.5.1 */}
