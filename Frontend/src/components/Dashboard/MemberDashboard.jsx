// src/components/Dashboard/MemberDashboard.jsx - FULLY FIXED VERSION
...
export default function MemberDashboard() {
  ...
  // UI State - Initialize from localStorage to avoid flashing
  const [running, setRunning] = useState(false);
  const [mode, setMode] = useState(() => {
    const saved = localStorage.getItem("IMALI_MODE");
  ...
  // Live Trading State - Enhanced with open positions tracking
  ...
  // Enhanced fetchLiveStats with open positions
  ...
  // fetchLiveTrades - no dependency on hasLiveConnection
  ...
  // fetchBotStatus with proper mode setting
  ...
  // FIXED: stopLiveBot with proper confirmation handling
  const stopLiveBot = useCallback(async () => {
    const activeExchange =
      botStatus?.exchange ||
      (integrationStatus.okx_mode === "live" ? "okx" :
    ...
    try {
      ...
      // Refresh state regardless of result to get latest DB status
      ...
      // Even if backend didn't confirm, check if bot is actually gone
      const newStatus = await BotAPI.getTradingBotStatus?.(true);
      const stillRunning = newStatus?.data?.some(b => 
        b.exchange === activeExchange && (b.isRunning || b.status === "running")
      );
      ...
      alert("Stop request was sent, but the backend did not confirm the bot stopped. Please try again or check your exchange dashboard.");
      return false;
    } catch (err) {
  ...
  }, [botStatus, integrationStatus, fetchBotStatus, fetchLiveStats, fetchLiveTrades, fetchExchangeBalances]);

  // FIXED: handleModeChange with proper async stop
  const handleModeChange = useCallback(async (newMode) => {
    ...
    if (running) {
      const stopped = mode === "live" ? await stopLiveBot() : true;
      if (!stopped) return;
      setRunning(false);
      if (intervalRef.current) clearInterval(intervalRef.current);
    }
  ...
  }, [running, mode, canUseLiveMode, stopLiveBot]);
  ...
  useEffect(() => {
    if (mode === "live" && canUseLiveMode) {
      ...
      balanceIntervalRef.current = setInterval(refreshData, BALANCE_REFRESH_INTERVAL);
  ...
  // FIXED: startStopBot with proper async handling
  const startStopBot = useCallback(async () => {
    if (running) {
      if (mode === "live") {
        await stopLiveBot();
      } else {
    ...
    await startLiveBot();
  }, [running, mode, canUseLiveMode, hasLiveConnection, stopLiveBot]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-black text-white">
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-4 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className={`${card} ${mode === "live" ? "border-emerald-500/50" : ""}`}>
            ...
            {liveStats.openPositions > 0 && (
              <div className="mt-2 text-xs text-cyan-400">📊 {liveStats.openPositions} open position{liveStats.openPositions !== 1 ? 's' : ''}</div>
            )}
          </div>
        </div>
        ...
        {/* Step 1: Choose Strategy - disabled when running */}
        <div className={card}>
            ...
            {STRATEGIES.map((item) => {
              ...
              return (
                <button key={item.id} onClick={() => unlocked && !running && setCurrentStrategy(item)} disabled={!unlocked || running}
                  ...
                  {running && <div className="mt-2 text-xs text-yellow-400">⏸️ Pause to change</div>}
                </button>
              );
            })}
          </div>
              ...
              {integrationStatus.okx_connected && (
                <div className="mt-2 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-3 text-xs text-emerald-300">
                  ...
                  {integrationStatus.okx_api_key_masked && (
                    <div className="mt-1 text-white/50">Key: {integrationStatus.okx_api_key_masked}</div>
                  )}
                </div>
              )}
              ...
              {integrationStatus.alpaca_connected && (
                <div className="mt-2 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-3 text-xs text-emerald-300">
                  ...
                  {integrationStatus.alpaca_api_key_masked && (
                    <div className="mt-1 text-white/50">Key: {integrationStatus.alpaca_api_key_masked}</div>
                  )}
                </div>
              )}
              <div className="mt-3 flex gap-2">
        ...
        <div className="text-center">
          <button
            ...
            disabled={isBotStarting || isInitialLoad}
