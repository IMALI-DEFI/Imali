...
const AUTOMATION_JOBS = [
  ...
  },
  {
    ...
    icon: '📧',
    defaultSchedule: '0 10 * * 1,3,5', // Mon, Wed, Fri at 10 AM
    channels: ['email'],
    lastRun: null,
    nextRun: null,
    status: 'active',
  ...
  },
  {
    ...
    icon: '📢',
    defaultSchedule: '* * * * *', // Every minute (checks for due announcements)
  ...
  },
  {
    ...
    icon: '📱',
    defaultSchedule: '0 12,18 * * *', // 12 PM and 6 PM daily
    channels: ['telegram'],
    lastRun: null,
    nextRun: null,
    status: 'active',
...
export default function MarketingAutomation() {
  ...
  const fetchJobs = useCallback(async () => {
    try {
      const response = await adminFetch('/api/admin/automation/jobs');
      if (response.ok) {
  ...
  const handleToggle = async (jobId) => {
    try {
      const response = await adminFetch('/api/admin/automation/jobs/toggle', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        showToast(`Job ${jobId} ${jobs.find(j => j.id === jobId).status === 'active' ? 'paused' : 'resumed'}`, 'success');
      }
    } catch (error) {
  ...
  const handleRunNow = async (jobId) => {
    try {
      const response = await adminFetch('/api/admin/automation/jobs/run', {
        method: 'POST',
        body: JSON.stringify({ jobId })
      });
      ...
      if (response.ok) {
        ...
        showToast(`Job ${jobId} executed successfully`, 'success');
  ...
  const handleSaveSchedule = async (jobId, schedule) => {
    try {
      const response = await adminFetch('/api/admin/automation/schedule', {
        method: 'POST',
  ...
  const handleViewLogs = async (jobId) => {
    try {
      const response = await adminFetch(`/api/admin/automation/logs/${jobId}`);
      if (response.ok) {
        const data = await response.json();
        setLogs(data.logs || []);
  ...
  const handleTestPost = async (jobId, channel, template) => {
    try {
      const response = await adminFetch('/api/admin/automation/test', {
        method: 'POST',
