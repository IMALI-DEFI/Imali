// src/pages/Billing.jsx - Production ready (Pro/Elite only, two-step payment)
...
export default function Billing() {
  ...
  // Billing model selection (before payment)
  ...
  useEffect(() => {
    if (!loading && alreadyPaid && tierInfo.requiresPayment) {
      navigate(tierInfo.redirectTo || "/activation", {
        replace: true,
        state: { tier, fromBilling: true, alreadyPaid: true },
      });
    }
  }, [loading, alreadyPaid, tier, navigate, tierInfo.redirectTo, tierInfo.requiresPayment]);

  // Step 1: Check card status only (no SetupIntent)
  const loadBillingState = useCallback(async () => {
    ...
    if (alreadyPaid) { setHasCard(true); setLoading(false); return; }
  ...
  }, [email, user?.email, isAdmin, user?.is_admin, tier, activation, alreadyPaid, navigate, tierInfo.redirectTo]);

  useEffect(() => {
    loadBillingState();
  }, [loadBillingState]); // No billingModel dependency → no extra intents

  // Step 2: User confirmed model → create SetupIntent
  ...
  }, [navigate, refreshActivation, refreshUser, tier, tierInfo.redirectTo, billingModel, profitSharePct]);
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <button
                          ...
                          className={`p-4 rounded-xl border text-left transition-all ${
                            billingModel === "fixed" ? "border-emerald-500/50 bg-emerald-500/10 ring-1 ring-emerald-500/30" : "border-white/10 bg-white/5 hover:bg-white/10"
                          }`}
                        >
                        ...
                        <button
                          ...
                          className={`p-4 rounded-xl border text-left transition-all ${
                            billingModel === "profit_share" ? "border-emerald-500/50 bg-emerald-500/10 ring-1 ring-emerald-500/30" : "border-white/10 bg-white/5 hover:bg-white/10"
                          }`}
                        >
                ...
                <>
                  ...
                  {!clientSecret ? (
                    ...
                    <StripeElements
                      ...
                      buttonLabel={billingModel === "profit_share" ? `Save Card for ${profitSharePct}% Profit Share` : `Add Card for ${tierInfo.label} Plan`}
                      tier={tier}
                    />
                  )}
                </>
