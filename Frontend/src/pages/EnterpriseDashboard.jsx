...
function Toast({ message, type = "info", onClose }) {
  ...
  const tone = type === "error" ? "border-red-200 bg-red-50 text-red-900" : type === "success" ? "border-green-200 bg-green-50 text-green-900" : "border-blue-200 bg-blue-50 text-blue-900";
  return (
...
export default function EnterpriseDashboard() {
  ...
  const loadDashboardData = useCallback(async ({ silent = false, force = false } = {}) => {
    ...
    try {
      ...
      // Fetch user's trading state from BotAPI
      ...
      // Generate chart series from analytics data
      if (analyticsResult.data?.members) {
        const dailyData = [];
        const memberTrades = analyticsResult.data.members;
        // Aggregate daily performance
        const dateMap = new Map();
        memberTrades.forEach(member => {
  ...
  const handleTogglePaperTrading = async (enabled) => {
    ...
    try {
      // Use BotAPI if available, otherwise mock
  ...
  if (loading || enterpriseLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 p-6 text-center">
        <div>
          <div className="text-xl font-extrabold text-slate-950 sm:text-2xl">Loading enterprise dashboard…</div>
          <div className="mt-2 text-sm font-semibold text-slate-500">Getting organization data, trades, and stats.</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-3 py-4 text-slate-950 sm:p-6">
      ...
      <div className="mx-auto max-w-7xl space-y-5 sm:space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {[
            ...
            <Link key={action.path} to={action.path} className={`rounded-xl border border-${action.color}-200 bg-${action.color}-50 p-3 text-center hover:shadow-md transition-all`}>
              <div className="text-2xl">{action.icon}</div>
              <div className="mt-1 text-xs font-bold text-slate-700">{action.title}</div>
            </Link>
          ))}
        </div>
        ...
        <div className="grid gap-5 xl:grid-cols-3">
          ...
          <Card>
            ...
            <div className="h-64 sm:h-72"><Doughnut data={doughnutData} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { color: "#0f172a", font: { weight: "bold" } } } } }} /></div>
          </Card>
        </div>
        ...
        <Card>
          ...
          <div className="space-y-2">
            {members.slice(0, 5).map((member, idx) => (
              <div key={member.user_id || idx} className="flex items-center justify-between p-3 rounded-xl border border-slate-200 bg-slate-50">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold">{member.email?.[0]?.toUpperCase() || "U"}</div>
                  <div>
            ...
            {members.length === 0 && <p className="text-center text-slate-500 py-4">No team members yet. Invite your team to get started.</p>}
            {members.length > 5 && <div className="text-center text-sm text-slate-500 pt-2">+{members.length - 5} more members</div>}
          </div>
        </Card>
      ...
      {/* API Keys Modal - Simplified for enterprise */}
      {showApiModal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 p-0 sm:items-center sm:p-4">
          <div className="max-h-[94vh] w-full overflow-auto rounded-t-3xl bg-white p-4 shadow-2xl sm:max-w-2xl sm:rounded-3xl sm:p-6">
            <div className="mb-5 flex items-start justify-between gap-4 border-b border-slate-200 pb-4">
              <div>
                <h2 className="text-xl font-extrabold text-slate-950 sm:text-2xl">Connect Organization API Keys</h2>
                <p className="mt-1 text-sm font-medium text-slate-600">Add OKX for crypto and Alpaca for stocks. These keys will be used for organization-wide trading.</p>
              </div>
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <h3 className="mb-4 text-lg font-extrabold">📈 Alpaca — Stocks</h3>
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-slate-500 text-center">API keys are encrypted. Contact support for setup assistance.</p>
                </div>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <h3 className="mb-4 text-lg font-extrabold">🔷 OKX — Crypto</h3>
                <div className="space-y-3">
                  <p className="text-sm text-slate-600">Paper Trading Keys (recommended to start)</p>
                  <Button variant="secondary" onClick={() => setShowApiModal(false)} className="w-full">Configure in Settings</Button>
                  <p className="text-xs text-slate-500 text-center">API keys are encrypted. Contact support for setup assistance.</p>
      ...
      {showLiveConfirm && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 p-0 sm:items-center sm:p-4">
          <div className="w-full rounded-t-3xl bg-white p-5 shadow-2xl sm:max-w-md sm:rounded-3xl sm:p-6">
            ...
            <p className="mt-3 text-sm font-semibold text-slate-600">Live trading uses real money through your organization's connected exchange accounts.</p>
