...
const API = {
  ...
  checkout: `${API_BASE}/api/billing/create-checkout`, // your unified billing endpoint
};

// Update STRATEGIES to match TradeDemo terminology (backend values)
const STRATEGIES = [
...
export default function SignupForm() {
  ...
  // Strategy stored as backend value (momentum, mean_reversion, ai_weighted, volume_spike)
  ...
  // OPTIONAL: promo status (first 50, etc)
  useEffect(() => {
    let mounted = true;
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      // 2) Billing setup for ALL tiers (Starter must collect card-on-file for performance fee)
      // Backend should return checkoutUrl (for subscription) or setupUrl (for starter setup mode)
      const checkoutResponse = await axios.post(
        API.checkout,
        {
          ...
          strategy,
          execution_mode,
          // optional metadata flags your backend can use
          promo_opt_in: true,
          promo_hint: "first50",
        },
        { withCredentials: true, timeout: 15000 }
      ...
      if (!checkoutResponse.data?.ok) {
        throw new Error(
          checkoutResponse.data?.detail || "Billing setup failed"
        );
      }
      ...
      // Nice UX: confetti, then redirect to Stripe
      ...
      if (error.response) {
        ...
        errorMessage =
          data?.detail || data?.error || data?.message || "Server error";
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-gray-300">
            Pick a plan • Pick a strategy • Add billing • Connect API • Start
            trading
          </p>

          <div className="mt-3 text-sm text-emerald-200">
            Promo: <b>First 50 users</b> get <b>5% fee over 3%</b> for{" "}
            <b>90 days</b>. Cancel anytime.
          </div>
          ...
          <div className="mt-2 text-[11px] text-white/40">API: {API_BASE}</div>
        </div>
        ...
        <div className="grid lg:grid-cols-5 gap-6">
          <aside className="lg:col-span-2 space-y-4">
            ...
            <div className="rounded-2xl bg-white/5 border border-white/10 p-5">
              <div
                className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold ${badgeStyle}`}
              >
              ...
              <div className="mt-3 flex items-center gap-3">
                ...
                <div>
                  <div className="font-bold">
                    {activeTier.base ? `${activeTier.monthly}` : "Free"}
                  </div>
              ...
              <div className="mt-4 text-sm text-white/80 space-y-2">
                <div>
                  ✅ {tier === "starter" ? "Auto Trading ONLY" : "Manual + Auto Trading"}
                </div>
                <div>✅ Cancel any time</div>
                <div className="text-xs text-amber-200/90">
                  All tiers require billing setup for fee collection
                </div>
              </div>

              <div className="mt-4 text-xs text-white/60">
                "Stocks and Established Crypto Trades"{" "}
                {tier === "elite" && "+ DEX Trading"}
              </div>
            </div>

            <div className="rounded-2xl bg-white/5 border border-white/10 p-5">
              <h3 className="font-bold mb-2">What happens after signup</h3>
              <ul className="text-sm text-white/80 space-y-1">
                <li>1. Secure billing setup (required for performance fees)</li>
                <li>2. Activation: connect OKX / broker / paper trading</li>
                <li>3. Choose your bot + strategy</li>
                <li>4. Launch Auto trading (Starter is Auto-only)</li>
              </ul>
              <div className="mt-2 text-xs text-white/60">
                You won’t access the dashboard until activation is complete.
              </div>
            </div>
          </aside>
            ...
            <div className="rounded-2xl bg-white/5 border border-white/10 overflow-hidden">
              ...
              <div className="p-5 border-b border-white/10">
                ...
                <div className="text-sm text-white/70 mt-1">
                  Create your account, then complete secure billing setup.
                </div>
              </div>
              ...
              <form onSubmit={submit} className="p-5 grid gap-4">
                ...
                <label className="block">
                  ...
                  <select
                    ...
                    className="w-full bg-black/30 border border-white/10 focus:border-emerald-400 outline-none p-3 rounded-xl"
                  >
                    <option value="starter">Starter — {TIERS.starter.monthly}</option>
                    <option value="pro">Pro — {TIERS.pro.monthly}</option>
                    <option value="elite">Elite — {TIERS.elite.monthly}</option>
                    <option value="stock">Stocks — {TIERS.stock.monthly}</option>
                    <option value="bundle">Bundle — {TIERS.bundle.monthly}</option>
                  </select>
                  <p className="text-xs text-white/60 mt-1">
                    {activeTier.fee} • {activeTier.description}
                  </p>
                </label>
                ...
                <div className="text-xs text-white/60 mb-2">
                  ...
                  <div className="mt-1 text-amber-200/90">
                    Billing setup is required for performance fee collection and promo eligibility.
                  </div>
                </div>
                ...
                >
                  {loading ? (
                    ...
                    "Continue to Secure Billing Setup"
                  )}
                </button>
