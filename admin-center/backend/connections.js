...
const S=require('./security'),P=require('./providers');
...
function safeError(e){return /^[A-Z][A-Z0-9_]{2,80}$/.test(e.message)?e.message:'SOCIAL_OPERATION_FAILED';}
...
function mount(app,{db,authenticateToken,requireAdmin}){
 ...
 router.get('/connections',wrap(async(req,res)=>{const {c}=loadConfig();const r=await db.query('SELECT * FROM social_connections_v1 ORDER BY brand,platform');res.json({success:true,automatic_publishing:false,connections:r.rows.map(x=>({...S.publicConnection(x),app_configured:configured(c,x.platform),callback_url:callback(c,x.platform),test_types:['x','facebook','linkedin'].includes(x.platform)?['text','link']:x.platform==='instagram'?['image','reel']:[],media_validation:'requires_connected_account'}))});}));
 router.post('/:brand/:platform/connect',wrap(async(req,res)=>{
  ...
  const expected=String(req.body.expected_account_id||'');if(!expected||expected.length>160)throw Error('EXPECTED_ACCOUNT_ID_REQUIRED');
  const old=(await db.query('SELECT account_id,generation FROM social_connections_v1 WHERE brand=$1 AND platform=$2',[b,p])).rows[0];if(old?.account_id&&old.account_id!==expected)throw Error('DESTINATION_MISMATCH');
 ...
 router.get('/queue',wrap(async(req,res)=>{const r=await db.query('SELECT id,brand,platform,account_id,state,platform_post_id,published_at,public_url,error,provider_state,created_at FROM social_queue_v1 ORDER BY created_at DESC LIMIT 100');res.json({success:true,automatic_publishing:false,items:r.rows});}));
 ...
 app.get('/api/social/callback/:platform',async(req,res)=>{
  ...
  try{const p=req.params.platform;S.validate('imali',p);const state=String(req.query.state||'');if(!/^[A-Za-z0-9_-]{43}$/.test(state))throw Error('STATE_INVALID');const {c,key}=loadConfig(),digest=S.hash(state),browser=cookie(req,'social_oauth_'+state.slice(0,12));
   ...
   const t=await P.exchange(p,c.apps[p],callback(c,p),code,S.open(key,st.verifier_cipher,digest));const who=await P.identify(p,c.apps[p],t,st.expected_account);if(String(who.id)!==st.expected_account)throw Error('DESTINATION_MISMATCH');
