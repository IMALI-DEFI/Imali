...
const getIntegrationStatus = async (skipCache = false) =>
  cachedGet("integrations", 10000, async () => {
    ...
    return {
      ...
      alpaca_auth_type: d.alpaca_auth_type || null,
      alpaca_api_key_masked: d.alpaca_api_key_masked || "",
      alpaca_account_masked: d.alpaca_account_masked || "",
      alpaca_mode: mode(d.alpaca_mode),
      alpaca_connected_at: d.alpaca_connected_at || null,
...
const getTradingActivity = async (
  exchange = "",
  limit = 100
) => {
  const params = new URLSearchParams();
  ...
  return getData(
    await api.get(
      `/api/trading/activity?${params.toString()}`
    )
  );
};
...
const BotAPI = {
  ...
  updateUserStrategy,
  getUserStrategyPreference: async () => {
    const result = getData(await api.get("/api/trading/strategies"));
    return {
      success: true,
      data: {
        strategyId:
